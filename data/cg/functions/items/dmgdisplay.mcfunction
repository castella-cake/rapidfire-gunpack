...
scoreboard players operation $hittype_temp cg.dmgdisp_temp = @s cg.bul_hittype.temp
...
execute if entity @s store result score $hp_temp cg.dmgdisp_temp run data get entity @s Health 100
...
execute as @s at @s if score $hittype_temp cg.dmgdisp_temp matches 1 run playsound entity.player.hurt master @s ~ ~ ~ 1 1 1
execute as @s at @s if score $hittype_temp cg.dmgdisp_temp matches 2 run playsound entity.player.hurt master @s ~ ~ ~ 1 1.5 1
...
#execute as @s run tag @s add cg.bul.hit
...
#execute as @a if score @s cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 1 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp run tellraw @a [{"text":""},{"selector":"@s","color":"red"},{"text":"は"},{"selector":"@s","color":"red"},{"text":"に倒された"}]
#execute as @a if score @s cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 2 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp run tellraw @a [{"text":""},{"selector":"@s","color":"red"},{"text":"は"},{"selector":"@s","color":"red"},{"text":"にヘッドショットで倒された"}]
