...
scoreboard players set $hittype_temp cg.dmgdisp_temp 1
...
execute as @a if score @s cg.player_number = $temp cg.dmgdisp_temp run data modify storage cg:dmgdisp_text text set value '[{"score":{"name":"@s","objective":"cg.dmgdisp"}},{"text":" DMG"}]'
...
execute as @a if score @s cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 2 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp run data modify storage cg:dmgdisp_text text set value '[{"text":"Killed ","color":"red"},{"score":{"name":"@s","objective":"cg.dmgdisp"},"color":"yellow"},{"text":" DMG","color":"white"}]'
#execute at @a if score @p cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 1 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp run tellraw @a [{"text":""},{"selector":"@s","color":"red"},{"text":"は"},{"selector":"@p","color":"red"},{"text":"にショットガンで倒された"}]
execute as @a if score @p cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 1 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp if score $connect_rfsys cg.config matches 1 run function rf:kill_bonus
