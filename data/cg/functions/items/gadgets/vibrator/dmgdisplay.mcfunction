scoreboard players operation $temp cg.dmgdisp_temp = @s cg.bul_pn.temp
...
execute if entity @e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1] store result score $hp_temp cg.dmgdisp_temp run data get entity @e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1] Health 100
#data modify storage cg:debug dmgdisp_debug set value '[{"nbt":"Health","entity":"@e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1,type=!player]","interpret":true}]'
...
execute as @e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1] at @s if score $hittype_temp cg.dmgdisp_temp matches 1 run playsound entity.player.hurt master @s ~ ~ ~ 1 1 1
execute as @e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1] at @s if score $hittype_temp cg.dmgdisp_temp matches 2 run playsound entity.player.hurt master @s ~ ~ ~ 1 1.5 1
...
execute as @e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1] run tag @s add cg.bul.hit
...
#execute as @a if score @s cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 1 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp run tellraw @a [{"text":""},{"selector":"@e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1]","color":"red"},{"text":"は"},{"selector":"@s","color":"red"},{"text":"に倒された"}]
#execute as @a if score @s cg.player_number = $temp cg.dmgdisp_temp if score $hittype_temp cg.dmgdisp_temp matches 2 if score $hp_temp cg.dmgdisp_temp <= $dmgr_temp cg.dmgdisp_temp run tellraw @a [{"text":""},{"selector":"@e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1]","color":"red"},{"text":"は"},{"selector":"@s","color":"red"},{"text":"にヘッドショットで倒された"}]
...
#tellraw @a [{"nbt":"Health","entity":"@e[distance=..5,type=!#cg:ignore,sort=nearest,limit=1,type=!player]"},{"text":"HP(NBT), "},{"score":{"name":"$hp_temp","objective":"cg.dmgdisp_temp"}},{"text":"HP(X100), "},{"score":{"name":"$dmgr_temp","objective":"cg.dmgdisp_temp"}},{"text":"DMG(RAW)"}]
